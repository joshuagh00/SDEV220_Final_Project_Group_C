...
from datetime import datetime  # for history list
...
##*******************************************************************##
class History:
    def __init__(self, location='stockroom', age = 'New', condition ='good'):
        self.log = []
        self.time = datetime() 
        self.location = location
        self.age = age  # a string, assume valid values are New, used
        self.condition = condition  # a string, assume valid values are good, bad
        self.log.append((location, self.time, age, condition)) ## append a fixed tuple
        
    def update(self, location='stockroom', age = 'New', condition ='good'):  
        self.log.append((datetime(), location,  age, condition)) ## append a fixed tuple
  
    def str(self):   ## create a string with item attributes
        str_ = 'Item history: \n'
        for i in len(self.log):
            str_ += ' Time: %s  Location: %s   Condition: %s, %s \n' \
                % (self.log[i][0],self.log[i][1],self.log[i][2],self.log[i][3] )
        return str_
